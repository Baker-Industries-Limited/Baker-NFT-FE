...
export default function Mint(props) {
  ...
  const checkFilter = (name) => {
    ...
    if (selectRef.current.value === "BakerFarmNFT ($100)") {
      res = back.filter(
        (item) =>
          item.nftaddress === "0xadC47F74c9262110A2eabFD43e164c70835dD98C"
      );
      setNFT(res);
    } else if (selectRef.current.value === "BakerFarmNFT ($200)") {
      res = back.filter(
        (item) =>
          item.nftaddress === "0x943a154e3B7b206159787f06E966B3afe09508f8"
      );
      setNFT(res);
    } else {
      setNFT(back);
    ...
    <div>
      ...
      <main>
        <section className="mint-section1">
          <div className="mint_text1">Mint 20 Baker NFTs</div>
